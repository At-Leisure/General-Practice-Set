...
# 项目根路径
root_dir = r'E:\File\DeskTop\expand\工具箱盒\模拟Explorer'
...
class Viewer(QWidget):
    ...
    def __init__(self, os_type: OSType = None, init_dir: str = None, *args, **kwargs):
        ...
        self.ui = uic.loadUi('./ui/view.ui', self)  # 动态加载ui文件
    ...
    def _select_icon(self, dir_name: str, dir_type) -> str:
        ...
        **函数名前缀有下划线时应视为该类的私有方法**"""
        if dir_type is DirType.File:  # 文件
            ...
            if suffix == 'py':
                path = root_dir + '/ico/file_type_python.svg'
            elif suffix == 'txt':
                path = root_dir + '/ico/file_type_text.svg'
            else:
                path = root_dir + '/ico/default_file.svg'
        else:  # 文件夹
            if dir_name == 'theme':
                path = root_dir + '/ico/folder_type_theme.svg'
            elif dir_name == 'utils':
                path = root_dir + '/ico/folder_type_tools.svg'
            else:
                path = root_dir + '/ico/default_folder.svg'
        return path
