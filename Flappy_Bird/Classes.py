...
class Bird(pygame.sprite.Sprite):
    ...
    def __init__(self, location, screen: pygame.Surface):
        ...
        self.angle = 0
        self.image1 = pygame.image.load(imgsPath.BlueBird1)    # 加载图片
        self.image2 = pygame.image.load(imgsPath.BlueBird2)
        self.image3 = pygame.image.load(imgsPath.BlueBird3)
...
class PlrTop(pygame.sprite.Sprite):
    ...
    def __init__(self, location):
        # 调父类来初始化子类
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load(imgsPath.pillar)    # 加载图片
        self.rect = self.image.get_rect()       # 获取图片rect区域
        self.rect.topleft = location      # 设置位置
...
class Pause(pygame.sprite.Sprite):
    ...
    def __init__(self, fullsize):
        # 调父类来初始化子类
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load(imgsPath.pause)    # 加载图片
        self.rect = self.image.get_rect()       # 获取图片rect区域
...
class ScoreDisplay():
    def __init__(self, location, screen: pygame.Surface, *, init_num=0) -> None:
        self.num = init_num
        self.pointedScreen = screen
        self.imageGroup = [pygame.image.load(
            imgsPath.score[i]) for i in range(10)]
